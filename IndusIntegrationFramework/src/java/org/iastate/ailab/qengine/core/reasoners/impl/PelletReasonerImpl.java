...
// new ends
/**
 * @author Roshan, Rohit This class has all the methods to call Pellet
 * Reasoner API to find super, sub, equivalent classes.
 */
public class PelletReasonerImpl extends DefaultReasonerImpl {
   ...
   /*
    * OntologyMapStore is not required incase of Pellet Reasoner as the
    * reasoner can directly raed the ont map files in owl format
    */
    ...
    * Init._this().getViewContext().getOntologyStore();
    */

   @Override
   public Set<URI> getEquivalentClass(OWLReasoner reasoner,
      ...
      try {
         ...
         if ((equivalentFlag != null)
               && (!equivalentFlag.equalsIgnoreCase("TRUE"))) {
   ...
   ZExpression inp) {
      ...
      try {
         ...
         /*
          * We can easily get a list of inconsistent classes. (A class is
          * inconsistent if it can't possibly have any instances). Note
          * that the getInconsistentClasses method is really just a
          * convenience method for obtaining the classes that are
...
//          System.out.println("There are no inconsistent classes");
         ...
         OWLClass owlClassObject = manager.getOWLDataFactory().getOWLClass(
               URI.create(classID.toString()));
